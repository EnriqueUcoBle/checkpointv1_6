	<div class="row">
	  				<div class="col-md-9">
						<div class="content-box-large">
			  				<div class="panel-heading">
					            <div class="panel-title">Info Usuario</div>
					          
					            <div class="panel-options">
					              <a href="update_info" data-rel="collapse"><i class="glyphicon glyphicon-refresh"></i></a>
					            </div>
					        </div>
			  				<div class="panel-body">
			  					<form class="form-horizontal" role="form">
								  <fieldset>						
										<div class="form-group">
											<label>Nombre</label>
											<input class="form-control"  readonly="readonly" placeholder="nombre del usuario" type="text">
										</div>
                    <div class="form-group">
											<label>Apelllidos</label>
										<input class="form-control" readonly="readonly" placeholder="apellidos del usuario" type="text">
										</div>
                    <div class="form-group">
											<label>Direccion</label>
											<input class="form-control " placeholder="direccion" type="text">
										</div>
										<div class="form-group">
											<label>Sexo:</label>
											<div class="col-md-12">
												<select class="form-control" id="select-1">
													<option>masculino</option>
													<option>femenino</option>
													<option>otro</option>
												</select> 
											</div>
										</div>
                    <div class="form-group">
											<label>Fecha naciemiento</label>
											<input class="form-control" placeholder="fecha de nacimiento" type="date">
										</div>
										 <div class="form-group">
											<label>Telefono </label>
											<input class="form-control" placeholder="ingrese su nuevo numero" type="text" name="telefono">
										</div>
										 <div class="form-group">
											<label>Correo </label>
											<input class="form-control" placeholder="ingrese su nuevo correo" type="email" name="correo">
										</div>
                    <div class="form-group">
											<label>Nueva Contraseña </label>
											<input class="form-control" placeholder="ingrese su nueva contraseña" type="text" name="password1">
										</div>
										<div class="form-group">
											<label>Confirmar Contraseña </label>
											<input class="form-control" placeholder="respita su nueva contraseña" type="text" name="password2">
										</div>
										<div class="form-group">
											<label>Confirme su cambios con la contraseña actual </label>
											<input class="form-control" placeholder="contraseña actual" type="text" name="password2">
										</div>
									</fieldset>
									<div>
										<div class="btn btn-primary">
											<i class="fa fa-save"></i>
											  Actualizar Datos  
										</div>
									</div>
								</form>
							
			  				</div>
			  			</div>
	  				</div>
			</div>