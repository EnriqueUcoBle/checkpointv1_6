	<div class="row">
	  				<div class="col-md-9">
						<div class="content-box-large">
			  				<div class="panel-heading">
					            <div class="panel-title">Resumen General del Alumno</div>
					          
					            <div class="panel-options">
					              <a href="profile_cap" data-rel="collapse"><i class="glyphicon glyphicon-refresh"></i></a>
					            </div>
					        </div>
			  				<div class="panel-body">
									<div class="panel-title">Kardex</div>
									<table class="table table-hover">
									<thead>
										<tr>
											<th scope="col">Materia</th>
											<th scope="col">Promedio</th>
											<th scope="col">Recursado</th>
											<th scope="col">Cursando</th>
										</tr>
									</thead>
									<tbody>
											<tr>
												<th scope="row">$Data_Request Not Aviable</th>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
											</tr>
									</tbody>
									</table>
									
			  				</div>
									<div class="panel-body">
									<div class="panel-title">Reportes</div>
									<table class="table table-hover">
									<thead>
										<tr>
											<th scope="col">Fecha</th>
											<th scope="col">Docente o Resposable</th>
											<th scope="col">Descripcion</th>
										
										</tr>
									</thead>
									<tbody>
											<tr>
												<th scope="row">$Data_Request Not Aviable</th>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												
											</tr>
									</tbody>
									</table>
			  				</div>
								<div class="panel-body">
									<div class="panel-title">Calificaiones del semestres</div>
									<table class="table table-hover">
									<thead>
										<tr>
											<th scope="col">Materia</th>
											<th scope="col">Parcial 1</th>
											<th scope="col">Parcial 2</th>
											<th scope="col">Parcial 3</th>
											<th scope="col">Examen final</th>
											<th scope="col">Promedio final</th>
										
										</tr>
									</thead>
									<tbody>
											<tr>
												<th scope="row">$Data_Request Not Aviable</th>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												
											</tr>
									</tbody>
									</table>
									
			  				</div>
									<div class="panel-body">
									<div class="panel-title">Asistencias</div>
									<table class="table table-hover">
									<thead>
										<tr>
											<th scope="col">Materia</th>
											<th scope="col">Asistencias totales</th>
											<th scope="col">Asisencias</th>
											<th scope="col">Inasisencias</th>
												<th scope="col">Detalles</th>
											
										</tr>
									</thead>
									<tbody>
											<tr>
												<th scope="row">$Data_Request Not Aviable</th>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												<td>$Data_Request Not Aviable</td>
												<td><button type="button" class="btn btn-danger">Ver Detalles</button></td>
																						
											</tr>
									</tbody>
									</table>
									
			  				</div>
			  			</div>
	  				</div>
			</div>